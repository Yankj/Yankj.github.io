...
category: ai
tags: [agent, postgresql, drizzle, zod, vercel-ai-sdk, data-types, tech-stack]
---
...
> 这条数据流对应 Agent 推理的**三阶段管线**：步骤 1-2 属于 **Stage 1（外部检索与装载）**——外部 Embedding 模型 + 数据库完成资料检索与上下文拼接；步骤 3 属于 **Stage 2（LLM 前向计算）**——Prompt 经 Tokenizer 分词后进入 Transformer 多层计算 $y = f(x; \Theta)$，产出全词表概率分布；步骤 3 的采样属于 **Stage 3（采样与自回归）**——采样器从概率分布选 Token，拼回输入循环生成，遇到 Tool Call 则触发 Agent Loop。三阶段通过**纯文本字符串**交接，外部 Embedding 的坐标空间与 LLM 内部空间完全隔离。
