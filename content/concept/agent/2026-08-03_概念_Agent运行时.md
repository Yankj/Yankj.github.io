---
publish: true
title: "2026-08-03 Agent 运行时：Skill 加载、Tool 调用与执行全过程"
date: "2026-08-03"
modified: "2026-08-03"
section: knowledge
knowledgeType: concept
category: ai
tags: [agent, runtime, skill, tool]
---
...
> **Agent Loop vs LLM 自回归循环**：Agent Loop 是应用层概念——每轮调用一次 LLM API，检查是否有 Tool Call，有则执行工具并回传结果。LLM 内部还有一层更底层的循环——**自回归循环（Autoregressive Loop）**：每生成一个 Token，拼回输入序列继续预测下一个 Token，直到输出完整回答或触发 Tool Call 指令。一次 API 调用 = 一次自回归循环 = 生成一段文本或一个 Tool Call。Agent Loop 是外层循环（多轮交互），自回归循环是内层循环（逐 Token 生成）。
...
- [Supabase](https://supabase.com/) — 认证与数据存储
- [OpenAI API：Function Calling](https://platform.openai.com/docs/guides/function-calling) — Tool 调用机制
- [Server-Sent Events (SSE)](https://developer.mozilla.org/zh-CN/docs/Web/API/Server-sent_events) — 流式响应协议
